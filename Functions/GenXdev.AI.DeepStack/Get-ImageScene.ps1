    ...
    begin {
        ...
        if (-not $NoDockerInitialize) {
            Microsoft.PowerShell.Utility\Write-Verbose `
                ("Ensuring DeepStack scene recognition service is available")
        ...
        function Format-SceneResult {
            ...
            $scene = if ($SceneData.label) { $SceneData.label } else { "unknown" }
            $confidence = if ($SceneData.confidence) { $SceneData.confidence } else { 0.0 }
